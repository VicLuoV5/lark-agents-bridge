 ...
 * `~/.feishu-codex-bridge/secrets-getter` (always user-owned, never a symlink)
 * so the config can refer to encrypted local secrets without a plaintext
 * App Secret. The wrapper internally `exec`s the real node + bridge with
 * `secrets get`. Bridge itself short-circuits the spawn and reads the
...
export async function ensureSecretsGetterWrapper(): Promise<string> {
  ...
  const content =
    `#!/bin/sh\n` +
    `# Auto-generated by lark-agents-bridge. Do not edit.\n` +
